...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{G9_calibration.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{G8_forest_plot.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{G10_inter_model_scatter.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{G11_paired_diff.png}
...
The compression results above raise a natural question: can the bias be corrected post hoc? We investigate whether a simple learned calibration function can recover the lost scoring range. We split the 150 source articles into a training set (112 articles, 7{,}140 samples) and a held-out test set (28 articles, 1{,}860 samples), stratified by category, and fit two calibration functions on the training set: an affine transform $\hat{S} = aS_{\text{raw}} + b$ and a four-parameter sigmoid $\hat{S} = d + c / (1 + e^{-a(S_{\text{raw}} - b)})$. We evaluate on the held-out test set to avoid overfitting.

Table~\ref{tab:calibration} reports the results. Affine calibration reduces the root-mean-square error (RMSE) against the ideal score by 27\% for GPT-5~mini ($2.56 \to 1.87$) and 12\% for Gemini~3~Flash ($1.82 \to 1.60$). The sigmoid offers negligible further improvement over the affine for both models, indicating that the compression is well approximated as a linear distortion rather than a non-linear warping. Critically, rank-order correlations (Kendall $\tau$, Spearman $\rho$) are unchanged by calibration, as expected for monotonic transforms---the models' relative ordering of texts is unaffected.

\begin{table}[t]
\centering
\caption{Calibration recovery on held-out test set (28 articles, 1{,}860 samples). RMSE is computed against the ideal score $S^* = 10(1 - \lambda)$.}
...
Figure~\ref{fig:calibrecovery} visualizes the effect: the affine-calibrated dose-response curve is pulled closer to the ideal line $S = 10(1 - \lambda)$, narrowing the shaded compression gap, though a residual error of 1.6--1.9 RMSE points remains even after calibration. This residual reflects the models' imperfect rank discrimination---their inability to linearly separate degradation levels---which no monotonic transform can correct.

\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{G15_calibration_recovery.png}
\caption{Dose-response curves before (solid) and after (dashed) affine calibration, against the ideal reference line (dotted black). Calibration narrows the gap between observed and ideal scores but cannot eliminate it entirely: the residual error reflects rank-order imprecision, not a correctable offset.}
\label{fig:calibrecovery}
\end{figure*}

These results carry a dual message. On the one hand, a simple two-parameter affine correction---requiring only a modest calibration set---recovers a meaningful fraction of the lost accuracy, making post-hoc calibration a practical and low-cost mitigation strategy. On the other hand, the substantial residual RMSE confirms that compression is not purely an offset problem: the models genuinely fail to spread scores across the full range, losing discriminative information that cannot be recovered by rescaling.
